...
from app.models.schemas import (
    ...
    ConversationHistory as SCHEMASCONVERSATIONHISTORY,
)
...
async def get_default_api_key():
    ...
    try:
        ...
        # 确保数据是正确的格式并且有至少一个密钥
        if (
            isinstance(api_keys_data, list)
            and len(api_keys_data) > 0
            and isinstance(api_keys_data[0], dict)
            and "key" in api_keys_data[0]
        ):
            # 返回第一个API密钥
...
async def update_model(
    model_id: int, model: ModelCreate, db: Session = Depends(get_db)
):
...
async def create_configuration(
    config: ConfigurationCreate, db: Session = Depends(get_db)
):
    try:
        # 验证所有模型是否存在且用途类型正确
        for step in config.steps:
            ...
            if not model:
                raise HTTPException(
                    status_code=404, detail=f"Model {step.model_id} not found"
                )
...
async def update_configuration(
    config_id: int, config: ConfigurationCreate, db: Session = Depends(get_db)
):
    try:
        # 获取现有配置
        db_config = (
            db.query(DBConfiguration).filter(DBConfiguration.id == config_id).first()
        )
        if not db_config:
...
async def get_configuration(config_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # 获取配置步骤
        db_config.steps = (
            db.query(ConfigurationStep)
            .filter(ConfigurationStep.configuration_id == config_id)
...
@app.post("/v1/conversations", response_model=SCHEMASCONVERSATIONHISTORY)
async def create_conversation(
    conversation: ConversationHistoryCreate, db: Session = Depends(get_db)
):
    db_conversation = ConversationHistory(**conversation.dict())
...
@app.get("/v1/conversations", response_model=List[SCHEMASCONVERSATIONHISTORY])
async def get_conversations(db: Session = Depends(get_db)):
    return db.query(ConversationHistory).all()


@app.get("/v1/conversations/{session_id}", response_model=SCHEMASCONVERSATIONHISTORY)
...
async def chat_completions(
    ...
    try:
        # 记录请求
        logger.info(
            f"接收到聊天补全请求: model={model}, messages_count={len(messages)}, stream={stream}"
        )
        ...
        if role_match:
            ...
            if stream:
                ...
                return StreamingResponse(
                    convert_coroutine_to_stream(result), media_type="text/event-stream"
                )
            else:
                # 普通响应
                logger.info("使用普通响应")
            ...
            try:
                ...
                if stream:
                    ...
                    response = StreamingResponse(
                        convert_coroutine_to_stream(
                            processor.process_request(
                                prompt, stream=True, meeting_id=meeting_id
                            )
                        ),
                    ...
                    response = await processor.process_request(
                        prompt, stream=False, meeting_id=meeting_id
                    )
                    return response
            except Exception as e:
        ...
        # 处理常规模型请求
            ...
            # 记录常规模型请求参数
            logger.info(
                f"处理常规模型请求: model={model}, enable_thinking={enable_thinking}, thinking_budget_tokens={thinking_budget_tokens}"
            )
            ...
            try:
                ...
                if db_model:
                    ...
                    if not config:
                        ...
                        logger.info(
                            f"未找到模型ID {model_id} 的配置，使用单一模型直接调用"
                        )
                        # 创建临时步骤
                        steps = [
                            {
                ...
                config = (
                    db.query(DBConfiguration)
                    .filter(
                        DBConfiguration.name == model, DBConfiguration.is_active == True
                    )
                    .first()
                )
            ...
            if not config:
                raise HTTPException(
                    status_code=404,
                    detail=f"No active configuration found for model {model}",
                )

            # 获取配置步骤
            steps = (
                db.query(ConfigurationStep)
                .filter(ConfigurationStep.configuration_id == config.id)
                .order_by(ConfigurationStep.step_order)
                .all()
            )
            ...
            if not steps:
                raise HTTPException(
                    status_code=404, detail="Configuration has no steps"
                )
...
async def convert_coroutine_to_stream(result_or_coroutine):
    ...
    """
    # 检查是否为协程
    if hasattr(result_or_coroutine, "__await__") and callable(
        result_or_coroutine.__await__
    ):
        try:
            # 如果是协程，先获取结果
