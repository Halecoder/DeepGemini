...
class ConfigurationStep(Base):
    ...
    configuration_id = Column(
        Integer, ForeignKey("configurations.id", ondelete="CASCADE")
    )
...
class Role(Base):
    ...
    discussion_groups = relationship(
        "DiscussionGroup", secondary=role_discussion_group, back_populates="roles"
    )
...
class DiscussionGroup(Base):
    ...
    # 关系
    roles = relationship(
        "Role", secondary=role_discussion_group, back_populates="discussion_groups"
    )
